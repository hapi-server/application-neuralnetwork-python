...
`extract_format_structured_data(data, parameters)`
...
`pyspedas_plotdata_to_hapidata(pyspedas_plotdata)`
...
`HAPINNTester(in_steps, out_steps, preprocess_func=None, preprocess_y_func=None)`
:   Initalizes PyTorch or Tensorflow Modules as well
    ...
    `forecast_plot(self, preds, pred_ndx, column_name, stride=None, return_data=False)`
    ...
    `plot(self, preds, pred_ndx, column_name, stride=None, return_data=False)`
    :   Creates a line plot to compare the ground truth and predicted
           values of a specific output column for one of the predicted outputs.
    ...
    `set_hapidatas(self, datas, xyparameters=None)`
...
`HAPINNTrainer(data_split, in_steps, out_steps, preprocess_func=None, preprocess_y_func=None, lag=True)`
:   A class for preparing HAPI data and putting it in
    ...
    `tf_train(self, model, epochs, batch_size=None, **kwargs)`
    ...
    `torch_train(self, model, loss_func, optimizer, epochs, device, batch_size=None, metric_func=None, verbose=1)`
    ...
    `train(self, model, epochs, batch_size=None, loss_func=None, metric_func=None, optimizer=None, device=None, verbose=1)`
